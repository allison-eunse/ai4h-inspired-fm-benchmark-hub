#!/usr/bin/env python3
"""
Run DNABERT-2 on DNA benchmarks with real embeddings.
"""
...
class DNABERT2Model:
    ...
    def __init__(self):
        self.name = "dnabert2_real"
        self.model_name = "zhihan1996/DNABERT-2-117M"
        
    def load(self):
        from transformers import AutoTokenizer, AutoModel
        print(f"[DNABERT-2] Loading {self.model_name}...")
        ...
        self.model.eval()
        print("[DNABERT-2] ✅ Model loaded!")
        
    def encode(self, sequences: list, batch_size: int = 32) -> np.ndarray:
        """Encode sequences using DNABERT-2 embeddings."""
        embeddings = []
...
def run_benchmark(model, dataset_name: str, data: dict, output_dir: Path):
    ...
    eval_yaml = {
        ...
        'run_metadata': {
            ...
            'encoding': 'dnabert2_transformer_embeddings',
...
def main():
    print("=" * 70)
    print("DNABERT-2 BENCHMARK")
    print("Using REAL transformer embeddings")
    print("=" * 70)
    ...
    for ds_name, ds_path in datasets.items():
        try:
            ...
            eval_file = evals_dir / f"eval_dna_dnabert2_real_{ds_name}.yaml"
            with open(eval_file, 'w') as f:
